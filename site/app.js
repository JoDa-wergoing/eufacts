...
function setMessage(text, kind = 'info') {
  if (!msg) return;
  msg.innerHTML = text
    ? `<div class="${kind === 'error' ? 'error' : 'foot'}">${text}</div>`
    : '';
}
...
function inferTimeUnit(points) {
  ...
  const allMonths = points.every(
    p => p.x instanceof Date && p.x.getUTCMonth() === 0
  );
  return allMonths ? 'year' : 'month';
}
          ...
          if (row.value === undefined) {
            if (pt.value_pct_gdp !== undefined) row.value = pt.value_pct_gdp;
              ...
              const numKey = Object.keys(row).find(
                k => k !== 'time' && Number.isFinite(toNum(row[k]))
              );
...
function detectKeys(rows) {
  if (!rows.length)
    return { timeKey: null, countryKey: null, numericKeys: [], geos: [] };
  ...
  const timeKey =
    keys.find(k =>
      ['time', 'TIME_PERIOD', 'period', 'date', 'year'].includes(k)
    ) ||
    keys.find(k => /time|period|date|year/i.test(k)) ||
    null;

  const countryKey =
    keys.find(k =>
      ['country', 'country_name', 'name', 'geo_label', 'country_code', 'geo', 'GEO'].includes(k)
    ) ||
    keys.find(k => /country|geo/i.test(k)) ||
    null;
  ...
  const geos = countryKey
    ? [...new Set(rows.map(r => r[countryKey]).filter(Boolean).map(String))]
    : [];
...
function summarizeShape(rows, timeKey) {
  const times = unique(
    rows.map(r => (timeKey ? String(r[timeKey]) : '')).filter(Boolean)
  );
...
function prettyMetricName(key) {
  const map = {
    value_pct_gdp: 'General government gross debt / tax (% of GDP)',
    value: 'Waarde',
...
function buildMultiSeries(rows, timeKey, valueKey, countryKey, selectedGeos) {
  ...
  for (const r of useRows) {
    ...
    if (x instanceof Date && !isNaN(x) && Number.isFinite(y)) {
      byGeo.get(geo).push({ x, y });
    }
  }
...
function buildBarRows(rows, countryKey, valueKey) {
  return rows
    .map(r => ({ label: labelForRow(r), y: toNum(r[valueKey]) }))
...
function getChartContext(periodText, isSinglePeriod) {
  const datasetLabel =
    datasetSelect?.selectedOptions?.[0]?.text || '';

  const viewRadio = document.querySelector('input[name="viewMode"]:checked');
  const viewMode = viewRadio?.value || (isSinglePeriod ? 'latest' : 'timeseries');

  let viewLabel;
  if (viewMode === 'latest') {
    viewLabel = periodText
      ? `Laatste periode (${periodText})`
      : 'Laatste periode';
  } else {
    viewLabel = 'Laatste periodes (tijdreeks)';
  }

  return { datasetLabel, viewLabel, viewMode };
}
...
function renderLineMulti(datasets, timeUnit, valueKey, ctxInfo) {
  const ctx = document.getElementById('chart');
  const data = { datasets };

  const titleParts = [];
  if (ctxInfo?.datasetLabel) titleParts.push(ctxInfo.datasetLabel);
  titleParts.push(prettyMetricName(valueKey));
  if (ctxInfo?.viewLabel) titleParts.push(ctxInfo.viewLabel);
...
function renderBar(items, valueKey, periodText, ctxInfo) {
  const ctx = document.getElementById('chart');
  const data = {
...
function renderTableFromDatasets(datasets) {
  ...
  let html =
    '<table><thead><tr><th>geo</th><th>periode (laatste)</th><th>waarde</th></tr></thead><tbody>';
  html += rows
    .map(
      r =>
        `<tr><td>${r.geo}</td><td>${r.period}</td><td>${r.value}</td></tr>`
    )
    .join('');
  html += '</tbody></table>';
...
function renderTableFromBars(items) {
  let html =
    '<table><thead><tr><th>land</th><th>waarde</th></tr></thead><tbody>';
  html += items
    .map(i => `<tr><td>${i.label}</td><td>${i.y}</td></tr>`)
    .join('');
  html += '</tbody></table>';
  tableWrap.innerHTML = html;
}
...
function computeUrlFromControls() {
  // Als ?data=... is meegegeven, geef die voorrang
  if (QUERY_DATA_OVERRIDE) return QUERY_DATA_OVERRIDE;

  const dsId =
    datasetSelect?.value && DATASETS[datasetSelect.value]
      ? datasetSelect.value
      : 'teina230';
  const dsCfg = DATASETS[dsId];

  const viewRadio = document.querySelector(
    'input[name="viewMode"]:checked'
  );
  const viewMode = viewRadio?.value || 'latest';

  if (viewMode === 'timeseries') return dsCfg.timeseries;
  return dsCfg.latest;
}

function updateDataUrlFromControls() {
  if (!dataUrlInput) return;
  const url = computeUrlFromControls();
...
async function load(url) {
  ...
  try {
    ...
    if (!Array.isArray(rows) || !rows.length)
      throw new Error('Lege of ongeldige dataset.');

    const { timeKey, countryKey, numericKeys, geos } = detectKeys(rows);
    if (!timeKey) throw new Error('Geen tijdsleutel gevonden.');
    if (!numericKeys.length)
      throw new Error('Geen numerieke kolommen gevonden.');

    // Serie-keuze
    const preferred = ['value_pct_gdp', 'value', 'OBS_VALUE'];
    let defaultKey =
      numericKeys.find(k => preferred.includes(k)) || numericKeys[0];

    // dropdown vullen met nette labels
    seriesKeySelect.innerHTML = numericKeys
      .map(
        k =>
          `<option value="${k}" ${
            k === defaultKey ? 'selected' : ''
          }>${prettyMetricName(k)}</option>`
      )
      .join('');
    ...
    const prevSelection = new Set(
      [...geoSelect.selectedOptions].map(o => o.value)
    );
    geoSelect.innerHTML = geos
      .map(
        g =>
          `<option value="${g}" ${
            prevSelection.has(g) ? 'selected' : ''
          }>${g}</option>`
      )
      .join('');

    const { times, singlePeriod, period } = summarizeShape(rows, timeKey);
    const ctxInfo = getChartContext(period, singlePeriod);

    // SINGLE PERIOD → BAR (latest view, geen landen-selectie)
    if (singlePeriod && countryKey) {
      const bars = buildBarRows(rows, countryKey, valueKey);
      if (!bars.length)
        throw new Error('Geen waarden om te tonen (bar).');

      // geo-select uitschakelen in deze mode
      if (geoSelect) {
        geoSelect.disabled = true;
        geoSelect.classList.add('is-disabled');
      }

      renderBar(bars, valueKey, period, ctxInfo);
      renderTableFromBars(bars);
      meta.innerHTML = `Bron: <code>${url}</code> · Records: <b>${
        rows.length
      }</b> · Periode: <b>${period}</b> · Kolom: <code>${valueKey}</code>`;
      setMessage('');
      return;
    }

    // MULTI-PERIOD → LINE
    // geo-select weer inschakelen
    if (geoSelect) {
      geoSelect.disabled = false;
      geoSelect.classList.remove('is-disabled');
    }

    await ensureTimeAdapter();

    const selectedGeos = [...geoSelect.selectedOptions].map(o => o.value);
    let datasets = buildMultiSeries(
      rows,
      timeKey,
      valueKey,
      countryKey,
    ...
    if (!datasets.length) {
      ...
      const auto = geos.length
        ? prio.filter(p => geos.includes(p)).concat(geos).slice(0, 5)
        : [];
      geoSelect.innerHTML = auto
        .map(g => `<option value="${g}" selected>${g}</option>`)
        .join('');
      datasets = buildMultiSeries(
        rows,
        timeKey,
        valueKey,
        countryKey,
    ...
    if (!datasets.length)
      throw new Error('Kon geen tijdreeks afleiden uit de data.');

    const allPoints = datasets.flatMap(ds => ds.data);
    const unit = inferTimeUnit(allPoints);
    renderLineMulti(datasets, unit, valueKey, ctxInfo);
    renderTableFromDatasets(datasets);
    meta.innerHTML = `Bron: <code>${url}</code> · Records: <b>${
      rows.length
    }</b> · Kolom: <code>${valueKey}</code> · Reeksen: <b>${
      datasets.length
    }</b>`;
    setMessage('');
  } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
  // CSS helper voor disabled select (zorg dat je in index.html iets als
  // select.is-disabled { opacity:.5; pointer-events:none; } hebt)
  if (dataUrlInput) {
    if (QUERY_DATA_OVERRIDE) {
      dataUrlInput.value = QUERY_DATA_OVERRIDE;
    } else {
      updateDataUrlFromControls();
    }
  }
  ...
  document
    ...
    ?.addEventListener('click', () =>
      load(dataUrlInput.value || computeUrlFromControls())
    );
